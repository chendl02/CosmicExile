...
public class Venus_to_Space : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        targetSceneName = "Solar System";
        player = GameObject.Find("Player").transform;
    }

    void Update()
    {
        if ( taskController.currentState == TaskState.Completed)
        {
            canEnterRocket = true;
        }
        // 计算玩家与目标点的距离
    ...
    void OnGUI()
    {
        if (isPlayerNear)
        {
            ...
            {
                GUI.Label(new Rect(Screen.width / 2 - 150, Screen.height / 2 - 20, 300, 40), "<color=red>You need connect all 4 ThunderArea first!</color>");
            }
        }
    }
